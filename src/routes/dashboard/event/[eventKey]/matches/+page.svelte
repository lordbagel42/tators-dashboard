<script lang="ts">
    ...
    const findMatch = (match: TBAMatch, matchScouting: Scouting.MatchScoutingData[], team: number) => {
        return matchScouting.find(m => m.data.matchNumber === match.match_number && m.data.compLevel === match.comp_level && m.data.team === team);
    }
</script>
...
{#snippet teamLink(teamKey: string, color: 'red' | 'blue', match: TBAMatch)}
    ...
    >
        <a 
            href="/dashboard/event/{data.event.key}/team/{team(teamKey)}"
