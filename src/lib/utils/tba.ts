...
import { EventSchema, MatchSchema, TeamSchema, type TBAEvent as E, type TBAMatch as M, type TBATeam as T, teamsFromMatch } from "tatorscout/tba";
import { z } from "zod";
...
export class TBAEvent {
    ...
    getMatches() {
        return attemptAsync(async () => {
            ...
            const matches = (await get('/tba/event/' + this.tba.key + '/matches', z.array(MatchSchema))).unwrap();
    ...
    getTeams() {
        return attemptAsync(async () => {
            ...
            const teams = (await get('/tba/event/' + this.tba.key + '/teams', z.array(TeamSchema))).unwrap();
