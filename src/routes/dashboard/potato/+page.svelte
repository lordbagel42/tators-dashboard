...
{#snippet row(data: {
    ...
    <tr class:you={data.username === you?.username}>
        <td>
            <img src="/potato/{Potato.getPhase(data.level)}.png" alt="{Potato.getPhase(data.level)}" srcset="" style="width: 56px; height: 56px;" title="{capitalize(Potato.getPhase(data.level))}">
        </td>
        <td class="text-{color}">{data.username}
    ...
    <div class="row mb-3 pt-3">
        <div class="col">
            ...
            <p class="text-muted">
                ...
                <br>
                &nbsp; Pit Scouting: 20 levels
                <br>
                &nbsp; Taking Team Pictures: 5 levels
            </p>
            <button type="button" class="btn btn-primary" onclick={() => window.history.back()}> <i class="material-icons">arrow_back</i> Go Back </button>
        </div>
    </div>
    <div class="row mb-3">
